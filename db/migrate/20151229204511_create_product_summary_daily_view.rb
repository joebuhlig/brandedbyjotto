class CreateProductSummaryDailyView < ActiveRecord::Migration
	def self.up
	    execute <<-SQL
	    	CREATE VIEW product_daily_summaries AS
	    	(SELECT o.completed_at::date, substring(v.sku from 1 for (char_length(v.sku) - 1)) as sku, 
...
GROUP BY o.completed_at::date, substring(v.sku from 1 for (char_length(v.sku) - 1)), p.name)
UNION ALL
(SELECT o.completed_at::date, 
...
ORDER BY completed_at, sort, sku
	...
	def self.down
		execute "DROP VIEW product_daily_summaries"
	end
end