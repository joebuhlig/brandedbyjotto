class CreateProductSummaryYearlyView < ActiveRecord::Migration
	def self.up
	    execute <<-SQL
	    	CREATE VIEW product_yearly_summaries AS
	    	(SELECT date_trunc('year',o.completed_at) as completed_at, substring(v.sku from 1 for (char_length(v.sku) - 1)) as sku, 
...
GROUP BY date_trunc('year',o.completed_at), substring(v.sku from 1 for (char_length(v.sku) - 1)), p.name)
UNION ALL
(SELECT date_trunc('year',o.completed_at) as completed_at, 
...
LEFT JOIN spree_products p on v.product_id = p.id
WHERE state = 'complete' AND o.completed_at::date > '2015-11-24'
GROUP BY date_trunc('year',o.completed_at), 
...
ORDER BY date_trunc('year',o.completed_at), 
CASE substring(v.sku from char_length(v.sku) for char_length(v.sku)) WHEN 'C' THEN 'CEDARBOX' WHEN 'W' THEN 'WHITEBOX' WHEN 'N' THEN 'NOBOX' END)
ORDER BY completed_at, sort, sku
	...
	def self.down
		execute "DROP VIEW product_yearly_summaries"
	end
end
